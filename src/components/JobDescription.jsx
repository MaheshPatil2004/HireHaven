...
const JobDescription = () => {
    ...
    const applyJobHandler = async () => {
        ...
        // 2. Recruiters cannot apply
        if (user.role === 'recruiter') {
            toast.error("Recruiters cannot apply for jobs.");
            return;
        }

        // 3. Optional Profile Update Nudge
        ...
        if (!hasResume || !hasSkills) {
            const wantsToUpdate = window.confirm(
                "Your profile is incomplete! Adding skills and a resume greatly increases your chances of getting selected.\n\nClick 'OK' to update your profile, or 'Cancel' to apply anyway."
            );

            if (wantsToUpdate) {
                navigate("/profile"); 
                return; // Stop the application process and route to profile
            }
            // If they click Cancel, the code just skips this block and continues downward to apply.
        }
